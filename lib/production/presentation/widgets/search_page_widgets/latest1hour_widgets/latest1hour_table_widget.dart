    ...
    return BlocBuilder<Latest1HourCubit, Latest1HourState>(
      builder: (context, state) {
        ...
        return PaginatedDataTable2(
          columns: SortWidgetLists.sortTableColumnNames2,
          source: rows,
    ...
    switch (newState) {
      case Latest1HourStates.success:
        return DataRow(cells: [
          DataCell(Text("${rows[index]["key"][1]}")),
          DataCell(Text("${rows[index]["key"][0]}")),
          DataCell(Text("${rows[index]["doc_count"]}"))
        ], color: const MaterialStatePropertyAll(MainColors.color1));
      case Latest1HourStates.loading:
        return SortWidgetLists.loadingRow2;
      default:
        return SortWidgetLists.emptyRow2;
    }
  }
